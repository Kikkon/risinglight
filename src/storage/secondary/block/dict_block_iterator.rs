...
pub fn decode_dict_block(data: Block) -> (usize, Block, Block) {
    let mut buffer = &data[..];
    let rle_count_sum = buffer.get_u32() as usize;
    let rle_length = std::mem::size_of::<i32>()
        + (std::mem::size_of::<i32>() + std::mem::size_of::<i16>()) * rle_count_sum;
    let dict_count_sum = buffer.get_u32() as usize;
    let rle_block =
        data.slice(2 * std::mem::size_of::<u32>()..(rle_length + 2 * std::mem::size_of::<u32>()));
    let dict_block = data.slice((rle_length + 2 * std::mem::size_of::<u32>())..);
    ...
    B: BlockIterator<A>,
{
    pub fn new(
        ...
        let rle_iter: RleBlockIterator<
            crate::array::PrimitiveArray<i32>,
            PlainPrimitiveBlockIterator<i32>,
        > = RleBlockIterator::<I32Array, PlainPrimitiveBlockIterator<i32>>::new(
